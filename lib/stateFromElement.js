...
var EMPTY_BLOCK = new _draftJs.ContentBlock({
  ...
  text: '',
  type: _draftJsTools.BLOCK_TYPE.UNSTYLED,
...
var BlockGenerator = function () {
  ...
  _createClass(BlockGenerator, [{
    ...
    value: function getBlockTypeFromTagName(tagName, attr) {
      switch (tagName) {
        case 'li':
          {
            var parent = this.blockStack.slice(-1)[0];
            return parent.tagName === 'ol' ? _draftJsTools.BLOCK_TYPE.ORDERED_LIST_ITEM : _draftJsTools.BLOCK_TYPE.UNORDERED_LIST_ITEM;
          }
        case 'blockquote':
          {
            return _draftJsTools.BLOCK_TYPE.BLOCKQUOTE;
          }
        case 'h1':
          {
            return _draftJsTools.BLOCK_TYPE.HEADER_ONE;
          }
        case 'h2':
          {
            return _draftJsTools.BLOCK_TYPE.HEADER_TWO;
          }
        case 'h3':
          {
            return _draftJsTools.BLOCK_TYPE.HEADER_THREE;
          }
        case 'h4':
          {
            return _draftJsTools.BLOCK_TYPE.HEADER_FOUR;
          }
        case 'h5':
          {
            return _draftJsTools.BLOCK_TYPE.HEADER_FIVE;
          }
        case 'h6':
          {
            return _draftJsTools.BLOCK_TYPE.HEADER_SIX;
          }
        case 'pre':
          {
            return _draftJsTools.BLOCK_TYPE.CODE;
          }
        case 'p':
          {
            if (attr) {
              var value = attr.nodeValue.replace(/;/, '');
              switch (value) {
    ...
    value: function processBlockElement(element) {
      ...
      var type = this.getBlockTypeFromTagName(tagName, element.attributes[0]);
...
function addStyleFromTagName(styleSet, tagName) {
  switch (tagName) {
    ...
    case 'strong':
      {
        styleSet = styleSet.add(_draftJsTools.INLINE_STYLE.BOLD);
        break;
      }
    case 'i':
    case 'em':
      {
        styleSet = styleSet.add(_draftJsTools.INLINE_STYLE.ITALIC);
        break;
      }
    case 'ins':
      {
        styleSet = styleSet.add(_draftJsTools.INLINE_STYLE.UNDERLINE);
        break;
      }
    case 'code':
      {
        styleSet = styleSet.add(_draftJsTools.INLINE_STYLE.CODE);
        break;
      }
    case 'del':
      {
        styleSet = styleSet.add(_draftJsTools.INLINE_STYLE.STRIKETHROUGH);
        break;
      }
    case 'sub':
      {
