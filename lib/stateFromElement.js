...
var _draftJsTools2 = require('draft-js-tools');
...
var EMPTY_BLOCK = new _draftJs.ContentBlock({
  ...
  text: '',
  type: _draftJsTools2.BLOCK_TYPE.UNSTYLED,
...
var ELEM_TO_ENTITY = {
  a: function a(tagName, element) {
    ...
    if (ELEM_ATTR_MAP.hasOwnProperty(tagName)) {
      ...
      try {
        for (var _iterator = Object.keys(attrMap)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          ...
          var dataKey = attrMap[_attr];
          var dataValue = element.getAttribute(_attr);
          if (dataValue != null) {
    ...
    if (data.url != null) {
      return _draftJs.Entity.create(_draftJsTools2.ENTITY_TYPE.LINK, 'MUTABLE', data);
    }
  }
};
  ...
  _createClass(BlockGenerator, [{
    ...
    value: function getBlockTypeFromTagName(tagName) {
      switch (tagName) {
        case 'li':
          {
            ...
            return parent.tagName === 'ol' ? _draftJsTools2.BLOCK_TYPE.ORDERED_LIST_ITEM : _draftJsTools2.BLOCK_TYPE.UNORDERED_LIST_ITEM;
          }
        case 'blockquote':
          {
            return _draftJsTools2.BLOCK_TYPE.BLOCKQUOTE;
          }
        case 'h1':
          {
            return _draftJsTools2.BLOCK_TYPE.HEADER_ONE;
          }
        case 'h2':
          {
            return _draftJsTools2.BLOCK_TYPE.HEADER_TWO;
          }
        case 'h3':
          {
            return _draftJsTools2.BLOCK_TYPE.HEADER_THREE;
          }
        case 'h4':
          {
            return _draftJsTools2.BLOCK_TYPE.HEADER_FOUR;
          }
        case 'h5':
          {
            return _draftJsTools2.BLOCK_TYPE.HEADER_FIVE;
          }
        case 'h6':
          {
            return _draftJsTools2.BLOCK_TYPE.HEADER_SIX;
          }
        case 'pre':
          {
            return _draftJsTools2.BLOCK_TYPE.CODE;
          }
        case 'div':
          {
            if (attr) {
        ...
        default:
          {
            return _draftJsTools2.BLOCK_TYPE.UNSTYLED;
          }
      }
    }
  }, {
...
function canHaveDepth(blockType) {
  switch (blockType) {
    case _draftJsTools2.BLOCK_TYPE.UNORDERED_LIST_ITEM:
    case _draftJsTools2.BLOCK_TYPE.ORDERED_LIST_ITEM:
      {
        return true;
      }
    default:
...
function addStyleFromTagName(styleSet, tagName) {
  switch (tagName) {
    ...
    case 'strong':
      {
        styleSet = styleSet.add(_draftJsTools2.INLINE_STYLE.BOLD);
        break;
      }
    case 'i':
    case 'em':
      {
        styleSet = styleSet.add(_draftJsTools2.INLINE_STYLE.ITALIC);
        break;
      }
    case 'ins':
      {
        styleSet = styleSet.add(_draftJsTools2.INLINE_STYLE.UNDERLINE);
        break;
      }
    case 'code':
      {
        styleSet = styleSet.add(_draftJsTools2.INLINE_STYLE.CODE);
        break;
      }
    case 'del':
      {
        styleSet = styleSet.add(_draftJsTools2.INLINE_STYLE.STRIKETHROUGH);
        break;
      }
  }
  return styleSet;
